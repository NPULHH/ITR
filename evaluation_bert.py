...
def encode_data(model, data_loader, log_step=10, logging=print):
    ...
    with torch.no_grad():
        for i, (img_rcnn, img_grid, img_mask, region_mask, grid_mask, captions, ids) in enumerate(data_loader):
            model.logger = val_logger

            img_glo, cap_glo, img_emb, cap_emb = model.forward_emb(img_rcnn, img_grid, img_mask, region_mask, grid_mask, captions)

            if img_embs is None:
                img_glos = torch.zeros(len(data_loader.dataset), img_glo.size(1)).cuda()
                cap_glos = torch.zeros(len(data_loader.dataset), cap_glo.size(1)).cuda()
                img_embs = torch.zeros((len(data_loader.dataset), img_emb.size(1), img_emb.size(2))).cuda()
                cap_embs = torch.zeros((len(data_loader.dataset), cap_emb.size(1), cap_emb.size(2))).cuda()

            img_glos[ids] = img_glo
            cap_glos[ids] = cap_glo
            img_embs[ids] = img_emb
            cap_embs[ids] = cap_emb

            # img_glos[ids] = img_glo.data.cpu().numpy().copy()
            # cap_glos[ids] = cap_glo.data.cpu().numpy().copy()
            # img_embs[ids] = img_emb.data.cpu().numpy().copy()
            # cap_embs[ids] = cap_emb.data.cpu().numpy().copy()

            # sims = model.forward_sim(img_glos, cap_glos, img_embs, cap_embs)
        ...
        img_glo_new = img_glos[0:img_embs.size(0):5]
        img_emb_new = img_embs[0:img_embs.size(0):5]
        sims = model.forward_sim(img_glo_new, cap_glos, img_emb_new, cap_embs)
    ...
    return img_glos, cap_glos, img_embs, cap_embs, sims
...
def evalrank(model_path, data_path=None, split='dev', fold5=False, region_bbox_file=None, feature_path=None):
    ...
    img_glos, cap_glos, img_embs, cap_embs, sims = encode_data(model, data_loader)
...
def main():
    ...
    evalrank('/home/ubuntu/DSRAN/ACM_MM_expand/ACM_F30K/runs/' + opt.model + '/' + opt.name + ".pth.tar", data_path = opt.data_path, split="test", fold5=opt.fold, region_bbox_file=opt.region_bbox_file, feature_path=opt.feature_path)
